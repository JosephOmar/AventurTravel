import Reason1 from '../Icons/Reason/Reason1.astro'
import Reason2 from '../Icons/Reason/Reason2.astro'
import Reason3 from '../Icons/Reason/Reason3.astro'
import Reason4 from '../Icons/Reason/Reason4.astro'
import Reason5 from '../Icons/Reason/Reason5.astro'
import Reason6 from '../Icons/Reason/Reason6.astro'

export const CARD = [
  {
    image: 'https://www.tours-machupicchu-peru.com/wp-content/uploads/2021/11/Ica.jpg',
    title: 'Huacachina - Ica',
    price: 'S/200',
    link: '/FullDay/Huacachina',
    description: 'Descubre la encantadora oasis de Huacachina en Ica, rodeada de dunas de arena espectaculares, donde podrás disfrutar de actividades como paseos en buggies y sandboarding, así como de la belleza natural y la tranquilidad del entorno desértico.'
  }, {
    image: 'https://www.peru.travel/contenido/general/imagen/es/532/1.1/mullaks-misminay-turistas.jpg',
    title: "Mullak's - Cuzco",
    price: 'S/400',
    link: '/FullDay/Mullaks',
    description: "Embárcate en una experiencia única en Mullak's, ubicado en las impresionantes montañas de Cuzco. Sumérgete en la cultura local mientras exploras los paisajes montañosos, interactúas con la comunidad indígena y disfrutas de deliciosa comida tradicional."
  }, {
    image: 'https://www.peru.travel/contenido/uploads/deportes_arequipa_bicicleta_637558238305566014.jpg',
    title: 'Colca - Arequipa',
    price: 'S/300',
    link: '/FullDay/Colca',
    description: 'Adéntrate en el cañón más profundo del mundo, el Colca en Arequipa, donde serás testigo de majestuosos cóndores sobrevolando los imponentes picos de las montañas. Disfruta de aguas termales relajantes y de la rica cultura andina que habita esta región.'
  }, {
    image: 'https://www.paquetesdeviajesperu.com/wp-content/uploads/2022/07/paracas-peru.jpg',
    title: 'Paracas - Ica',
    price: 'S/250',
    link: '/FullDay/Paracas',
    description: 'Explora la belleza natural y la vida marina abundante en Paracas, un paraíso costero en Ica. Navega por las aguas turquesas de la Reserva Nacional de Paracas, admira los impresionantes acantilados y conoce la fascinante historia geológica de la región.'
  }, {
    image: 'https://www.paquetesdeviajesperu.com/wp-content/uploads/2022/07/nazca-peru.jpg',
    title: 'Nazca - Ica',
    price: 'S/350',
    link: '/FullDay/Nazca',
    description: 'Deslúmbrate con las misteriosas líneas de Nazca en Ica, un sitio arqueológico declarado Patrimonio de la Humanidad por la UNESCO. Sobrevuela estas enigmáticas figuras geométricas y zoomorfas que se extienden por el desierto, mientras te sumerges en la historia y el misterio de esta antigua civilización.'
  }, {
    image: 'https://elcomercio.pe/resizer/8mcNgiIeuop_a_3urmGFtdkPuow=/620x0/smart/filters:format(jpeg):quality(75)/cloudfront-us-east-1.images.arcpublishing.com/elcomercio/A6G4GWKOXRBCXF5BDC3VSNRPDY.jpg',
    title: 'Antioquía',
    price: 'S/450',
    link: '/FullDay/Antioquia',
    description: 'Sumérgete en la historia y la cultura de Antioquía, un encantador pueblo enclavado en los Andes peruanos. Recorre sus calles empedradas, visita sus iglesias coloniales y disfruta de la hospitalidad de sus habitantes mientras te maravillas con las impresionantes vistas panorámicas de las montañas circundantes.'
  }, {
    image: 'https://elcomercio.pe/resizer/SbUANyo_ltbOsG9nwrrTxf9kv3w=/620x0/smart/filters:format(jpeg):quality(75)/cloudfront-us-east-1.images.arcpublishing.com/elcomercio/4EXYALXF5BGF5HZ6ZKWCAZWH2A.jpg',
    title: 'Cordillera de la Viuda',
    price: 'S/150',
    link: '/FullDay/Cordillera-de-la-Viuda',
    description: 'Explora la impresionante Cordillera de la Viuda, un paraíso natural situado en la región central de Perú. Embárcate en emocionantes caminatas por senderos escénicos, admira cascadas cristalinas y sumérgete en la serenidad de este paisaje montañoso espectacular.'
  }
]

export const TOURS_NACIONAL = [
  {
    name: 'Amazonas',
    link: '/Tours-Nacional/amazonas'
  },
  {
    name: 'Ancash',
    link: '/Tours-Nacional/ancash'
  },
  {
    name: 'Arequipa',
    link: '/Tours-Nacional/arequipa'
  },
  {
    name: 'Chanchamayo',
    link: '/Tours-Nacional/chanchamayo'
  },
  {
    name: 'Cusco',
    link: '/Tours-Nacional/cusco'
  },
  {
    name: 'Ica',
    link: '/Tours-Nacional/ica'
  },
  {
    name: 'Iquitos',
    link: '/Tours-Nacional/iquitos'
  },
  {
    name: 'Lambayeque',
    link: '/Tours-Nacional/lambayeque'
  },
  {
    name: 'Piura',
    link: '/Tours-Nacional/piura'
  },
  {
    name: 'Puno',
    link: '/Tours-Nacional/puno'
  },
  {
    name: 'Tarapoto',
    link: '/Tours-Nacional/tarapoto'
  }
]
export const TOURS_INTERNATIONAL = [
  {
    name: 'Sudamérica',
    link: '/Tours-Internacional/Sudamerica'
  },
  {
    name: 'Caribe',
    link: '/Tours-Internacional/Caribe'
  },
  {
    name: 'Norteamérica',
    link: '/Tours-Internacional/Norteamerica'
  },
  {
    name: 'Europa',
    link: '/Tours-Internacional/europa'
  },
  {
    name: 'Asia',
    link: '/Tours-Internacional/asia'
  },
  {
    name: 'África',
    link: '/Tours-Internacional/africa'
  }
]
export const TOURS_LIMA = [
  {
    name: 'CityTour',
    link: '/Tours-Lima/city-tour'
  },
  {
    name: 'Tours Temáticos',
    link: '/Tours-Lima/tours-tematicos'
  },
  {
    name: 'Museos Religiosos',
    link: '/Tours-Lima/museos-religiosos'
  },
  {
    name: 'Museos Arqueológicos',
    link: '/Tours-Lima/museos-arqueologicos'
  },
  {
    name: 'Full Days',
    link: '/Tours-Lima/Full-Days'
  },
  {
    name: 'Weekend',
    link: '/Tours-Lima/Weekend'
  }
]

export const TOURS_ESCOLAR = [
  {
    name: 'Visita de Estudios',
    link: '/Tours-Escolares/Visita-De-Estudios'
  },
  {
    name: 'Viaje de Estudios',
    link: '/Tours-Escolares/Viaje-De-Estudios'
  },
  {
    name: 'Viaje de Promoción',
    link: '/Tours-Escolares/Viaje-De-Promocion'
  }
]

export const FATES = [
  {
    name: 'Perú',
    description: 'Desde las ruinas de Machu Picchu hasta la selva amazónica, Perú ofrece una aventura única. Explora la riqueza cultural, histórica y natural de este país fascinante.',
    img: '/img/Fates/Machu-Picchu.webp',
    option: 'md:w-[450px] w-[400px]',
    option2: '',
    link: '/Tours-Nacional'
  },
  {
    name: 'Internacional',
    description: 'Explora destinos internacionales emocionantes, desde ciudades cosmopolitas hasta paraísos tropicales. Amplía tus horizontes y crea recuerdos inolvidables en nuestros destinos internacionales.',
    img: '/img/Fates/Caribe.webp',
    option: 'md:w-[450px] w-[400px]',
    option2: '',
    link: '/Tours-Internacional'
  },
  {
    name: 'Lima',
    description: 'Descubre la rica fusión de historia, modernidad y gastronomía en la capital peruana, Lima. Explora playas animadas y sumérgete en la calidez de esta ciudad vibrante.',
    img: '/img/Fates/Lima.webp',
    option: 'md:w-[450px] w-[400px]',
    option2: '',
    link: '/Tours-Lima'
  },
  {
    name: 'Escolares',
    description: 'Ofrecemos promociones escolares diseñadas para grupos, proporcionando experiencias educativas y recreativas. Permitimos a los estudiantes explorar, aprender y disfrutar juntos en destinos cuidadosamente seleccionados.',
    img: '/img/Fates/Promocion.webp',
    option: 'md:w-[450px] w-[400px]',
    option2: '',
    link: '/Tours-Escolares'
  },
  {
    name: 'Viajes Especiales',
    description: 'Descubra experiencias únicas para empresas. Incentive y premie a sus colaboradores con destinos exclusivos y momentos inolvidables que fortalecerán el espíritu de equipo.',
    img: '/img/Fates/Especiales.webp',
    option: 'md:w-[920px] w-[400px]',
    option2: 'md:col-span-2 col-span-1',
    link: '/Tours-Especiales'
  }
]

const TAGS = {
  Reason1: {
    icon: Reason1
  },
  Reason2: {
    icon: Reason2
  },
  Reason3: {
    icon: Reason3
  },
  Reason4: {
    icon: Reason4
  },
  Reason5: {
    icon: Reason5
  },
  Reason6: {
    icon: Reason6
  }
}

export const REASONS = [
  {
    reason: 'Ahorro',
    description: 'Ofrecemos las mejores ofertas y descuentos para que puedas disfrutar de tus viajes sin comprometer tu presupuesto.',
    bgimg: [TAGS.Reason1]
  },
  {
    reason: 'Reservas Confirmadas',
    description: 'Garantizamos la confirmación inmediata de tus reservas, brindándote tranquilidad y certeza desde el momento de la planificación.',
    bgimg: [TAGS.Reason2]
  },
  {
    reason: 'Viaje Seguro',
    description: 'Priorizamos tu seguridad en cada aspecto del viaje, desde la selección de destinos hasta la implementación de protocolos para asegurar una experiencia sin preocupaciones.',
    bgimg: [TAGS.Reason3]
  },
  {
    reason: 'Atención Personalizada',
    description: 'Nuestro equipo dedicado está siempre a tu disposición, ofreciendo asesoramiento personalizado para crear itinerarios adaptados a tus preferencias y necesidades individuales.',
    bgimg: [TAGS.Reason4]
  },
  {
    reason: 'Experiencia',
    description: 'Con 20 años de experiencia en la industria, nuestra agencia de viajes ofrece un profundo conocimiento, confianza y expertise para brindar a nuestros clientes experiencias únicas y sin preocupaciones.',
    bgimg: [TAGS.Reason5]
  },
  {
    reason: 'Flexibilidad',
    description: 'Ofrecemos flexibilidad a medida para adaptarnos a tus necesidades. Cambia fechas, destinos o servicios con facilidad, asegurando que tus experiencias de viaje se ajusten perfectamente a tus preferencias.',
    bgimg: [TAGS.Reason6]
  }
]
